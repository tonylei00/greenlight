...
func ValidateEmail(v *validator.Validator, email string) {
	...
	v.Check(validator.Matches(email, validator.EmailRX), "email", "invalid email address")
}
...
func (m UserModel) Update(u *User) error {
	...
	args := []any{u.Name, u.Email, u.Password.hash, u.Activated, u.ID, u.Version}
...
func (m UserModel) GetForToken(token, scope string) (*User, error) {
...
func (m UserModel) GetForPermission(permissionCode string) ([]User, error) {
	query := `
		SELECT users.id, users.created_at, users.name, users.email, users.password_hash, users.activated, users.version
		FROM users
		INNER JOIN users_permissions ON users_permissions.user_id = user.id
	...
	rows, err := m.DB.QueryContext(ctx, query, permissionCode)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
